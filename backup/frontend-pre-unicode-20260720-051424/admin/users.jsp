...
<head>
    ...
    <title>Qu?n Lý Ngu?i Dùng — TourBuddy Admin</title>
...
<div class="dashboard-wrapper">
    ...
    <main class="main-content">
        ...
        <header class="top-header" style="margin-bottom: 24px;">
            <div>
                <h1 style="font-size: 24px; color: var(--gray-900); margin: 0 0 8px 0;">Danh sách ngu?i dùng</h1>
                <p style="color: var(--gray-500); margin: 0; font-size: 14px;">Qu?n lý tài kho?n ngu?i dùng, hu?ng d?n viên, nhân viên và qu?n tr? viên trong h? th?ng.</p>
            </div>
            <div class="header-actions">
                <button class="btn-modern btn-outline" onclick="window.location.reload()">
                    <i data-lucide="refresh-cw" style="width: 16px;"></i> Làm m?i
                </button>
                <button class="btn-modern btn-outline" id="exportUsersBtn">
                    <i data-lucide="download" style="width: 16px;"></i> Xu?t d? li?u
                </button>
                <button class="btn-modern btn-primary" id="addUserBtn">
                    <i data-lucide="plus" style="width: 16px;"></i> Thêm ngu?i dùng
                </button>
            </div>
        </header>
        ...
        <div class="modern-card">
            ...
            <div style="padding: 20px 24px; border-bottom: 1px solid var(--gray-200);">
                <div class="filter-bar" style="margin: 0;">
                    <div class="search-box">
                        ...
                        <input type="text" id="searchInput" placeholder="Tìm ki?m theo tên, email ho?c s? di?n tho?i...">
                    </div>
                    <div class="filter-group">
                        <select class="filter-select" id="roleFilter">
                            <option value="all">T?t c? vai trò</option>
                        ...
                        <select class="filter-select" id="statusFilter">
                            ...
                            <option value="locked">Ðã khóa</option>
                        </select>
                        <select class="filter-select">
                            <option>Ngày tham gia (M?i lúc)</option>
                            <option>Tháng này</option>
                            <option>Tu?n này</option>
                        </select>
                        <button class="btn-modern btn-primary" onclick="applyFilters()" style="padding: 10px 20px; height: 42px;">L?c</button>
                    </div>
                </div>
            </div>
            ...
            <div class="bulk-actions" id="bulkActionsPanel">
                <div style="font-weight: 600; color: var(--primary);">
                    Ðã ch?n <span id="selectedCount">0</span> ngu?i dùng
                </div>
                <div style="display: flex; gap: 8px;">
                    ...
                    <button class="btn-modern btn-outline" style="background: white; color: var(--danger);" onclick="bulkToggleStatus(false)"><i data-lucide="lock" style="width: 14px;"></i> Khóa tài kho?n</button>
                    <button class="btn-modern btn-outline" style="background: white; color: var(--success);" onclick="bulkToggleStatus(true)"><i data-lucide="unlock" style="width: 14px;"></i> M? khóa</button>
                ...
                <table class="modern-table" id="usersTable">
                    <thead>
                        <tr>
                            ...
                            <th>Tr?ng thái</th>
                            <th>Ngày tham gia</th>
                            <th style="text-align: right;">Hành d?ng</th>
                        </tr>
                    </thead>
                            ...
                            <tr data-role="<c:out value='${user.role.roleName}'/>" data-status="${user.isActive ? 'active' : 'locked'}" data-search="<c:out value='${user.fullName} ${user.email} ${user.phoneNumber}'/>">
                                ...
                                <td>
                                    <c:choose>
                                        <c:when test="${user.isActive}">
                                            <span class="badge status-active"><div style="width:6px;height:6px;border-radius:50%;background:currentColor;"></div> Ho?t d?ng</span>
                                        </c:when>
                                        <c:otherwise>
                                            <span class="badge status-locked"><div style="width:6px;height:6px;border-radius:50%;background:currentColor;"></div> Ðã khóa</span>
                                        </c:otherwise>
                                    </c:choose>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="row-actions" style="justify-content: flex-end;">
                                        <button class="action-icon" title="Xem chi ti?t" onclick="openDrawer('<c:out value='${user.userId}'/>')">
                                            <i data-lucide="eye" style="width: 18px;"></i>
                                        </button>
                                        <button class="action-icon" title="Ch?nh s?a vai trò" onclick="openSingleRoleModal('<c:out value='${user.userId}'/>')">
                                        ...
                                        <c:if test="${user.userId ne sessionScope.sessionUser.userId}">
                                            <form action="?action=toggleStatus" method="POST" style="margin:0;">
                                                ...
                                                <button type="button" class="action-icon <c:out value='${user.isActive ? "danger" : ""}'/>" title="<c:out value='${user.isActive ? "Khóa tài kho?n" : "M? khóa"}'/>" onclick="confirmToggleStatus(this.form, ${user.isActive})">
...
<div class="drawer" id="userDrawer">
    ...
    <div class="drawer-body" id="drawerBody">
        ...
        <div style="text-align: center; padding: 40px; color: var(--gray-500);">
            ...
            <p style="margin-top: 10px;">Ðang t?i thông tin...</p>
        </div>
    </div>
</div>
...
<div class="drawer-overlay" id="bulkRoleOverlay" style="display: none; align-items: center; justify-content: center; z-index: 2000;">
    <div style="background: white; padding: 24px; border-radius: 12px; width: 400px; box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1);">
        <h3 style="margin: 0 0 16px 0; font-size: 18px; color: var(--gray-900);">Gán vai trò hàng lo?t</h3>
        <form id="bulkRoleForm" action="?action=bulkAssignRole" method="POST">
            ...
            <div style="display: flex; justify-content: flex-end; gap: 12px;">
                <button type="button" class="btn-modern btn-outline" onclick="closeBulkRoleModal()">H?y</button>
                <button type="submit" class="btn-modern btn-primary">Luu thay d?i</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    function exportUsersCSV() {
        ...
        if (rows.length === 0) {
            showToast('Không có ngu?i dùng nào d? xu?t.', 'error');
            return;
        }
        ...
        showToast('Ðã xu?t ' + rows.length + ' ngu?i dùng.', 'success');
    }

    function openAddUserModal() {
        showToast('Ch?c nang t?o ngu?i dùng dang du?c phát tri?n. Vui lòng dùng form dang ký công khai.', 'warning');
    }
    ...
    function bulkToggleStatus(status) {
        ...
        if (confirm(`B?n có ch?c mu?n \${status ? 'm? khóa' : 'khóa'} \${checkedBoxes.length} tài kho?n này?`)) {
    ...
    function bulkDeleteUsers() {
        ...
        if (confirm(`B?n có th?c s? mu?n xóa vinh vi?n \${checkedBoxes.length} tài kho?n này? Hành d?ng này không th? hoàn tác và s? xóa toàn b? d? li?u h? so liên quan!`)) {
            const form = document.createElement('form');
            form.method = 'POST';
        ...
        fetch(`?action=api_get&id=\${userId}`)
            ...
            .then(res => {
                if(res.status === 'success') {
                    ...
                    drawerBody.innerHTML = `<div style="color: var(--danger); padding: 20px;">L?i: \${res.message}</div>`;
                }
            })
            .catch(err => {
                drawerBody.innerHTML = `<div style="color: var(--danger); padding: 20px;">L?i k?t n?i máy ch?!</div>`;
            });
    }
    ...
    function renderDrawer(user, stats) {
        ...
        const dob = user.profile?.dateOfBirth ? new Date(user.profile.dateOfBirth).toLocaleDateString('vi-VN') : 'Chua c?p nh?t';
        const address = user.profile?.address || 'Chua c?p nh?t';
        const interests = user.profile?.travelInterests || 'Chua c?p nh?t';
        ...
        drawerBody.innerHTML = `
            ...
            <div class="drawer-section">
                ...
                <div class="info-grid">
                    <div class="info-item"><label>S? di?n tho?i</label><span>\${user.phoneNumber || 'Chua c?p nh?t'}</span></div>
                    <div class="info-item"><label>Ngày sinh</label><span>\${dob}</span></div>
                    <div class="info-item"><label>Gi?i tính</label><span>\${user.profile?.gender || 'Chua c?p nh?t'}</span></div>
                    <div class="info-item"><label>Ð?a ch?</label><span>\${address}</span></div>
                </div>
            </div>

            <div class="drawer-section">
                <h4><i data-lucide="map" style="width:16px;"></i> Thông tin du l?ch</h4>
                <div class="info-grid" style="grid-template-columns: 1fr;">
                    <div class="info-item"><label>S? thích & Ho?t d?ng</label><span>\${interests}</span></div>
                </div>
            </div>

            <div class="drawer-section">
                <h4><i data-lucide="bar-chart-2" style="width:16px;"></i> Th?ng kê ho?t d?ng</h4>
                <div class="stats-boxes">
                    <div class="stat-box">
                        <span class="num">\${stats.trips}</span>
                        <span class="label">Chuy?n di</span>
                    </div>
                    <div class="stat-box">
                    ...
                    <div class="stat-box">
                        ...
                        <span class="label">Ðánh giá</span>
                    </div>
                    <div class="stat-box">
                        <span class="num">\${stats.companions}</span>
                        <span class="label">B?n d?ng hành</span>
                    </div>
                </div>
            </div>
        `;
    ...
    function confirmToggleStatus(form, isCurrentlyActive) {
        const actionText = isCurrentlyActive ? 'KHÓA' : 'M? KHÓA';
        if (confirm(`B?n có ch?c ch?n mu?n \${actionText} tài kho?n này?`)) {
            form.submit();
        }
    }
