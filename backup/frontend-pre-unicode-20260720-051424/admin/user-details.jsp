...
<head>
    ...
    <title>Chi Ti?t Ngu?i Dùng — TourBuddy Enterprise</title>
    ...
    <main class="main-content">
        <header class="top-header">
            ...
            <div class="header-right">
                <a href="${pageContext.request.contextPath}/admin/users" class="btn btn-secondary btn-sm" style="background:#f1f3f4; color:#333; padding: 8px 16px; border-radius:4px; text-decoration:none;">
                    <i data-lucide="arrow-left" style="width:16px; height:16px; vertical-align:middle;"></i> Quay l?i
                </a>
            </div>
        </header>
        ...
        <section class="view-panel active">
            <div class="detail-grid">
                    ...
                    <div style="text-align: center;">
                        ...
                        <div style="margin-top: 20px;">
                            <span class="badge ${user.isActive ? 'badge-active' : 'badge-locked'}">
                                ${user.isActive ? 'Tài kho?n Ðang Ho?t Ð?ng' : 'Tài kho?n B? Khóa'}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="detail-card">
                    <h3 style="margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 10px;">Thông Tin Chi Ti?t</h3>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                        ...
                        <div class="detail-item">
                            <div class="detail-label">S? Ði?n Tho?i</div>
                            <div class="detail-value">${user.phoneNumber}</div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label">Ðã Xác Th?c Email</div>
                            <div class="detail-value">${user.isVerified ? 'R?i' : 'Chua'}</div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label">Ngày Ðang Ký</div>
                            <div class="detail-value"><fmt:formatDate value="${user.createdAt}" pattern="dd/MM/yyyy HH:mm"/></div>
                        </div>
                        <div class="detail-item">
                            <div class="detail-label">L?n Ðang Nh?p Cu?i</div>
                            <div class="detail-value">
                                <c:choose>
                                    <c:when test="${not empty user.lastLoginAt}">
                                        <fmt:formatDate value="${user.lastLoginAt}" pattern="dd/MM/yyyy HH:mm"/>
                                    </c:when>
                                    <c:otherwise>Chua t?ng dang nh?p</c:otherwise>
                                </c:choose>
                            </div>
                        </div>
                    </div>
                    ...
                    <c:if test="${not empty user.profile}">
                        <h3 style="margin-top: 30px; margin-bottom: 20px; border-bottom: 1px solid #eee; padding-bottom: 10px;">H? So Cá Nhân</h3>
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                            <div class="detail-item">
                            ...
                            <div class="detail-item">
                                <div class="detail-label">Gi?i Tính</div>
                                <div class="detail-value">${user.profile.gender}</div>
                            </div>
                            <div class="detail-item" style="grid-column: span 2;">
                                <div class="detail-label">Ð?a Ch?</div>
