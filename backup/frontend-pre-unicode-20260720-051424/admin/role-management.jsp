...
<head>
    ...
    <title>Phân Quy?n & Vai Trò — TourBuddy Enterprise</title>
...
<div class="dashboard-wrapper">
    ...
    <main class="main-content">
        <header class="top-header">
            <h1>Phân Quy?n H? Th?ng</h1>
            <div class="header-right">
                <div class="header-search">
                    <i data-lucide="search"></i>
                    <input type="text" id="searchInput" placeholder="Tìm ki?m vai trò...">
                </div>
            ...
            <div class="role-management-container" style="display: flex; gap: 30px; margin-top: 20px; align-items: stretch; height: calc(100vh - 120px);">
                ...
                <div class="left-panel" style="width: 280px; background: rgba(22, 25, 50, 0.58); backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px); border-radius: 16px; padding: 20px; box-shadow: 0 8px 32px rgba(0,0,0,0.3); display: flex; flex-direction: column; border: 1px solid rgba(139, 92, 246, 0.2);">
                    ...
                    <div style="position: relative; margin-bottom: 15px;">
                        ...
                        <input type="text" id="roleSearch" placeholder="Tìm ki?m vai trò..." class="form-control" style="width: 100%; padding: 10px 10px 10px 35px;" onkeyup="filterRoles()">
                    </div>
                    ...
                    <div id="roleList" style="display: flex; flex-direction: column; gap: 8px; flex: 1; overflow-y: auto; margin-bottom: 15px; padding-right: 5px;">
                        <c:forEach var="role" items="${roles}">
                            <div class="role-item" data-role-id="${role.roleId}" data-role-name="${role.roleName}" data-role-desc="${role.description}" onclick="selectRole(${role.roleId}, this)" style="padding: 12px 15px; border-radius: 8px; background: rgba(255,255,255,0.03); color: #9fa9cb; cursor: pointer; border: 1px solid transparent; transition: all 0.2s; position: relative;">
                                ...
                                <div style="font-size: 12px; color: #64748b; margin-top: 4px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${empty role.description ? 'Chua có mô t?' : role.description}</div>
                    ...
                    <!-- role-actions: T?o / S?a / Xóa vai trò (UC19) -->
                    <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(139,92,246,0.15); display: flex; flex-direction: column; gap: 8px;">
                        <button type="button" onclick="openCreateRoleModal()" class="btn-forecast" style="width: 100%; padding: 10px; border-radius: 8px; font-weight: 600; cursor: pointer;">
                            <i data-lucide="plus-circle" style="width: 16px; vertical-align: text-bottom;"></i> T?o vai trò
                        </button>
                        <button type="button" id="editRoleBtn" onclick="openEditRoleModal()" class="btn-cancel" style="width: 100%; padding: 10px; border-radius: 8px; font-weight: 600; cursor: pointer; border: 1px solid rgba(139,92,246,0.3);">
                            <i data-lucide="edit-3" style="width: 16px; vertical-align: text-bottom;"></i> S?a vai trò
                        </button>
                ...
                <div class="right-panel" style="flex: 1; background: rgba(22, 25, 50, 0.58); backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px); border-radius: 16px; padding: 25px; box-shadow: 0 8px 32px rgba(0,0,0,0.3); border: 1px solid rgba(139, 92, 246, 0.2); display: flex; flex-direction: column;">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 25px;">
                        <div>
                            <h3 style="color: #f8fafc; margin-top: 0; margin-bottom: 5px; font-size: 20px; font-weight: 600;">Ma tr?n phân quy?n</h3>
                            <p style="color: #9fa9cb; margin: 0; font-size: 14px;">Thi?t l?p quy?n truy c?p cho vai trò du?c ch?n</p>
                        </div>
                        <div id="unsavedBadge" style="display: none; background: #fef3c7; color: #d97706; padding: 6px 12px; border-radius: 20px; font-size: 13px; font-weight: 600; align-items: center; gap: 6px;">
                            <span style="font-size: 10px;">?</span> Chua luu
                        </div>
                    </div>
                    ...
                    <form id="permissionForm" onsubmit="submitPermissions(event)" style="display: flex; flex-direction: column; flex: 1; overflow: hidden;">
                        ...
                        <div style="flex: 1; overflow-y: auto; padding-right: 5px;">
                            <table style="width: 100%; border-collapse: collapse;">
                                <thead style="position: sticky; top: 0; background: rgba(15,17,35,0.98); z-index: 10; border-bottom: 2px solid rgba(139,92,246,0.25);">
                                    <tr>
                                        <th style="padding: 15px; text-align: left; font-weight: 600; color: #9fa9cb;">Ch?c nang</th>
                        ...
                        <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid rgba(139,92,246,0.15); display: flex; justify-content: flex-end; gap: 12px; align-items: center;">
                            <span id="unsavedText" style="color: #F59E0B; font-weight: 500; font-size: 14px; display: none; margin-right: 10px;">
                                <i data-lucide="alert-triangle" style="width: 16px; vertical-align: text-bottom;"></i> B?n có thay d?i chua du?c luu
                            </span>
...
<div id="roleModal" class="modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(15, 23, 42, 0.5); backdrop-filter: blur(4px);">
    <div class="modal-content" style="margin: 10% auto; padding: 30px; width: 400px;">
        <h3 id="roleModalTitle" style="margin-top: 0; margin-bottom: 20px; color: #f8fafc; font-size: 20px; font-weight: 600;">T?o Vai Trò</h3>
        ...
        <form action="${pageContext.request.contextPath}/admin/roles" method="post">
            ...
            <div style="margin-bottom: 20px;">
                <label style="display: block; margin-bottom: 8px; font-weight: 500; color: #9fa9cb; font-size: 14px;">Mô T?</label>
                <textarea id="roleDescInput" name="description" rows="3" placeholder="Mô t? ch?c nang c?a vai trò..." class="form-control" style="width: 100%; resize: vertical;"></textarea>
            </div>
            
            <div style="display: flex; justify-content: flex-end; gap: 10px;">
                <button type="button" onclick="closeModal('roleModal')" class="btn-cancel" style="padding: 10px 20px;">H?y</button>
                <button type="submit" class="btn-forecast" style="width: auto; padding: 10px 20px;">Luu</button>
            </div>
        </form>
    </div>
</div>
...
<div id="deleteModal" class="modal" style="display: none; position: fixed; z-index: 1000; left: 0; top: 0; width: 100%; height: 100%; background-color: rgba(15, 23, 42, 0.5); backdrop-filter: blur(4px);">
    <div class="modal-content" style="margin: 15% auto; padding: 30px; width: 400px; text-align: center;">
        ...
        <p style="color: #cbd5e1; margin-bottom: 25px; font-size: 15px;">B?n có ch?c ch?n mu?n xóa vai trò này? Hành d?ng này không th? hoàn tác.</p>
        ...
        <form action="${pageContext.request.contextPath}/admin/roles" method="post" style="display: flex; justify-content: center; gap: 12px;">
            ...
            <button type="button" onclick="closeModal('deleteModal')" class="btn-cancel" style="flex: 1; padding: 10px;">H?y</button>
...
<script>
    ...
    const moduleNames = {
        'Tour Management': 'Qu?n lý Tour',
        'Booking Management': 'Qu?n lý Booking',
        'User Management': 'Qu?n lý Ngu?i dùng',
        'Role Management': 'Qu?n lý Vai trò',
        'Matching Management': 'Qu?n lý Ghép dôi b?n d?ng hành',
        'Request Management': 'Qu?n lý Yêu c?u k?t n?i',
        'Review Management': 'Qu?n lý Ðánh giá',
        'Payment Management': 'Qu?n lý Thanh toán',
        'System Settings': 'Báo cáo h? th?ng',
        'Content Management': 'Qu?n lý N?i dung'
    };
    ...
    // openCreateRoleModal — m? modal t?o vai trò, reset form v? ch? d? create.
    function openCreateRoleModal() {
        document.getElementById('roleModalTitle').textContent = 'T?o Vai Trò';
    ...
    // openEditRoleModal — m? modal s?a vai trò d?a trên vai trò dang du?c ch?n trong sidebar.
    function openEditRoleModal() {
        if (!currentRoleId) {
            showToast('warning', 'Vui lòng ch?n m?t vai trò d? s?a.');
            return;
        }
        const item = document.querySelector('.role-item.active');
        if (!item) {
            showToast('warning', 'Không tìm th?y thông tin vai trò dang ch?n.');
            return;
        }
        document.getElementById('roleModalTitle').textContent = 'S?a Vai Trò';
    ...
    // openDeleteRoleModal — m? modal xác nh?n xóa vai trò dang ch?n.
    function openDeleteRoleModal() {
        if (!currentRoleId) {
            showToast('warning', 'Vui lòng ch?n m?t vai trò d? xóa.');
            return;
        }
    ...
    function selectRole(roleId, el) {
        if (hasUnsavedChanges()) {
            if (!confirm("B?n có thay d?i chua du?c luu. B?n có ch?c mu?n chuy?n d?i vai trò?")) {
                return;
            }
        }
    ...
    function restoreDefaults() {
        if (confirm("Khôi ph?c quy?n m?c d?nh cho vai trò này? T?t c? d? li?u chua luu s? m?t.")) {
    ...
    function openRoleModal(mode) {
        ...
        if (mode === 'create') {
            document.getElementById('roleModalTitle').innerText = 'T?o Vai Trò M?i';
            document.getElementById('roleActionInput').value = 'createRole';
            document.getElementById('roleIdInput').value = '';
            document.getElementById('roleNameInput').value = '';
            document.getElementById('roleDescInput').value = '';
        } else {
            if (!el) return showToast('warning', 'Vui lòng ch?n vai trò d? s?a');
            document.getElementById('roleModalTitle').innerText = 'S?a Vai Trò';
            document.getElementById('roleActionInput').value = 'updateRole';
    ...
    function confirmDeleteRole() {
        if (!currentRoleId) return showToast('warning', 'Vui lòng ch?n vai trò d? xóa');
        document.getElementById('deleteRoleIdInput').value = currentRoleId;
        document.getElementById('deleteModal').style.display = 'block';
    }
    ...
    function submitPermissions(e) {
        ...
        .then(data => {
            if(data.success) {
                showToast('success', 'C?p nh?t phân quy?n thành công');
                const newPerms = [];
                ...
                showToast('error', 'Không th? c?p nh?t phân quy?n: ' + data.message);
            }
        })
        .catch(err => {
            showToast('error', 'Ðã x?y ra l?i m?ng!');
        })
        .finally(() => {
            btn.disabled = false;
            btn.innerText = 'Luu thay d?i';
        });
    }
